...
export async function createReadSwappedJob(
  name: string,
  sourceIndex: string,
  targetIndex: string,
    ...
    spec: {
      template: {
        spec: {
          containers: [
            {
              name: "read-swapped-worker",
              image: "busybox:latest",
              command: ["/bin/sh"],
              args: [
                "-c",
                `echo "Swapping read operations from ${sourceIndex} to ${targetIndex}" && sleep 8 && echo "Read operations swapped"`,
              ],
              env: [
                {
                  name: "SOURCE_INDEX",
  ...
  try {
    ...
    console.log(`Created job ${jobName} for read swap ${name}`);
  } catch (error) {
...
export async function handleReadSwapped(
  ...
  try {
    ...
    try {
      ...
      console.log(`Creating job for read swap ${name}`);
      await createReadSwappedJob(name, sourceIndex, targetIndex, jobsApi);
    }
    ...
    if (jobStatus === "succeeded") {
      console.log(
        `Job ${jobName} succeeded for ${name}, updating status to CLEANUP`,
      );
    ...
    console.error(`Error handling READ_SWAPPED status for ${name}:`, error);
  }
}
